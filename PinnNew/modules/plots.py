...
from modules.models import LOSS_RESIDUAL, LOSS_BOUNDARY, LOSS_BOUNDARY_Y, LOSS_INITIAL, LOSS_HAMIL, MEAN_ABSOLUTE_ERROR
...
def plot_KdV_model(model, xstart,length, time, lim1=-1, lim2 = 1, c= 0., t0 = None, u_exact = None, save_path = None, show=True) -> None:
    ...
    for i, t_cs in enumerate(t_cross_sections):
        ...
        if u_exact != None:
            plt.plot(x_flat, u_exact(tx), label = 'exact')
            plt.legend()
...
def plot_heat_model(model, xstart,length, time, save_path = None, show=True) -> None:
    """
...
def plot_training_loss(history, x_scale = "linear", y_scale = "linear", save_path=None, show=True):
    ...
    if LOSS_BOUNDARY_Y in history:
        if len(history[LOSS_BOUNDARY_Y]) > 0:
            plt.plot(history[LOSS_BOUNDARY_Y], label='boundary loss y', alpha=0.8)
