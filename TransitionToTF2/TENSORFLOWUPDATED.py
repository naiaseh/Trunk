...
class PhysicsInformedNN:
    ...
    def __init__(self, x0, u0, x1, u1, layers, dt, lb, ub, q):
        ...
        # Load IRK weights
        tmp = np.float32(np.loadtxt('/Users/chloeclohosey/Desktop/PINNs-master/Utilities/IRK_weights/Butcher_IRK%d.txt' % (q), ndmin = 2))
...
###########################
        ...
        optimizer = tf.optimizers.LBFGSB(maxiter=50000, maxfun=50000, maxcor=50, maxls=50, ftol=1.0 * np.finfo(float).eps)
        ...
        def optimization_step():
            with tf.GradientTape() as tape:
                loss = loss_fn()
        gradients = tf.GradientTape.gradient(self.loss, optimizer.variables())
...
if __name__ == "__main__": 
    ...
    data = scipy.io.loadmat('/Users/chloeclohosey/Desktop/PINNs-master/main/Data/KdV.mat')
    ...
    dt = np.isscalar(t_star[idx_t+skip] - t_star[idx_t])        
